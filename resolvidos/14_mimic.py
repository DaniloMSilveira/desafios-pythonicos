...
def mimic_dict(filename):
  ...
  # é utilizado o defaultdict visto ser um dicionário porém o mesmo consegue criar novas chaves quando não possui
  dt = defaultdict(list)
  # adicionando a palavra inicial
  dt[''] = [words[0]]
  # definindo o tamanho do dicionário, caso não realizar o procedimento abaixo ocorre erro de "list index out of range"
  dt[words[-1]] = ['']
  # preenche a lista de palavras para cada chave do dicionário imitador
  for i, word in enumerate(words[:-1]):
    dt[word].append(words[i-1])
    dt[word].append(words[i+1])
...
def print_mimic(mimic_dict, word):
  ...
  for cont in range(200):
      newWord = random.choice(mimic_dict[newWord])
      text += ' ' + newWord
